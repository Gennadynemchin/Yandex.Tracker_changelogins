...
class EntityUserManager:
    ...
    def search_entities(self, filter_field: str, entity_type: str, user_id: str, per_page: int = 100) -> List[List[str]]:
        ...
        try:
            ...
            logger.error("HTTP error during search %s/%s: %s", entity_type, filter_field, e)
        except requests.exceptions.ConnectionError:
            logger.error("Connection error during search %s/%s", entity_type, filter_field)
        except requests.exceptions.Timeout:
            logger.error("Timeout during search %s/%s", entity_type, filter_field)
        except Exception as e:
            logger.error("Unexpected error during search %s/%s: %s", entity_type, filter_field, e)
    ...
    def update_single_field(self, field: str, user_id: str, entity_type: str, entity_ids: list) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = requests.post(url, headers=self.creds.headers, data=data)
            response.raise_for_status()
            response_data = response.json()
            logger.info("Updated %s/%s for %d entities: %s", entity_type, field, len(entity_ids), response_data)
    ...
    def update_list_field(self, field: str, old_user_id: str, new_user_id: str, entity_type: str, entity_ids: list) -> tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            response = requests.post(url, headers=self.creds.headers, data=add_data)
            response.raise_for_status()
            add_result = response.json()
            logger.info("Added user to %s/%s for %d entities: %s", entity_type, field, len(entity_ids), add_result)
        except requests.exceptions.HTTPError as e:
            ...
            logger.error("Unexpected error adding user to %s/%s: %s", entity_type, field, e)
            return None, None
        ...
        remove_data = json.dumps({
            ...
            "values": {
                "fields": {
                    field: {"remove": [old_user_id]}
                }
            }
        })
        
        try:
            response = requests.post(url, headers=self.creds.headers, data=remove_data)
            response.raise_for_status()
            remove_result = response.json()
            logger.info("Removed user from %s/%s for %d entities: %s", entity_type, field, len(entity_ids), remove_result)
        except requests.exceptions.HTTPError as e:
            logger.error("HTTP error removing user from %s/%s: %s", entity_type, field, e)
        except requests.exceptions.ConnectionError:
            logger.error("Connection error removing user from %s/%s", entity_type, field)
        except requests.exceptions.Timeout:
            logger.error("Timeout removing user from %s/%s", entity_type, field)
        except Exception as e:
            logger.error("Unexpected error removing user from %s/%s: %s", entity_type, field, e)
    ...
    for user_line in users_list:
        ...
        for ent_type in entity_types:
            for fld in filter_fields:
                try:
                    entity_batches = manager.search_entities(fld, ent_type, old_usr_id, per_page=100)
                    
                    if not entity_batches or not any(entity_batches):
                        logger.debug("No entities found for %s/%s with user %s", ent_type, fld, old_usr_id)
                        continue
                    
                    total_entities = sum(len(batch) for batch in entity_batches)
                    logger.info("Found %d %s(s) with %s=%s", total_entities, ent_type, fld, old_usr_id)

                    for batch in entity_batches:
                        if not batch:
                            continue

                        logger.info("Updating batch of %d %s(s) for field %s...", len(batch), ent_type, fld)

                        if fld in SINGLE_VALUE_FIELDS:
                            res = manager.update_single_field(fld, new_usr_id, ent_type, batch)
                            if res:
