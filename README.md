...
- Скрипт `mapping_users.py` делает запрос списка пользователей организации с помощью метода `get_users`. Полученный список пользователей передается в `extract_users`, который обрабатывает полученный список, извлекает необходимые атрибуты пользователей и сортирует пользователей на два списка:
